...
func TestGetThreadByID(t *testing.T) {
	mockRepo := new(mocks.ForumRepository)
	mockThread := entity.Thread{ID: 1, Title: "Test Thread"}
	...
	t.Run("error", func(t *testing.T) {
		mockRepo.On("GetThreadByID", 2).Return(entity.Thread{}, errors.New("error")).Once()

		u := NewPostUseCase(mockRepo)
		thread, err := u.GetThreadByID(2)

		assert.Error(t, err)
		assert.Equal(t, entity.Thread{}, thread)
		mockRepo.AssertExpectations(t)
	})
}
...
func TestCreateThread(t *testing.T) {
	mockRepo := new(mocks.ForumRepository)
	validThread := entity.Thread{
		Title:   "Valid Title",
		Content: "Valid content",
		UserID:  1,
	}
	createdThread := entity.Thread{
		ID:      1,
		Title:   "Valid Title",
		Content: "Valid content",
		UserID:  1,
...
func TestDeleteThreadByID(t *testing.T) {
	mockRepo := new(mocks.ForumRepository)
	thread := entity.Thread{ID: 1, UserID: 1}
...
func TestDeletePostByID(t *testing.T) {
	mockRepo := new(mocks.ForumRepository)
	post := entity.Post{ID: 1, UserID: 1}
...
func TestEditThread(t *testing.T) {
	mockRepo := new(mocks.ForumRepository)
	thread := entity.Thread{ID: 1, Title: "New Title", Content: "New Content", UserID: 1}
